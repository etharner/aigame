 ...
 *
 * Keeps trying to read output from given bot input/error stream. Stores read value and
 * sets a bot's response to read value if it is not the error stream.
 *
            ...
            while (!finished && (lastLine = bufferedReader.readLine()) != null) {
                if (!lastLine.contains("VM warning") && buffer.length() < 1000000) { //catches bots that return way too much (infinite loop)
                    if (this.type.equals("output")) {
											...
											lastLine =  splitted[1];
											this.engine.players.get(normalId).response = lastLine;
										}
